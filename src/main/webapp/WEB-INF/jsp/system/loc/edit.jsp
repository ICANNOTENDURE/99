						...
						<form action="loc/saveOrUpdate.do" name="userForm" id="userForm" method="post">
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tr>
									...
									<td><input type="text" name="locName" id="loginname" value="${pd.locName}" placeholder="这里输入账号" title="账号" style="width:98%;" /></td>
								</tr>
								<tr>
									<td style="width:79px;text-align: right;padding-top: 13px;">状态:</td>
									<td>
										<select name="locStatus" title="状态">
										<option value="Y" <c:if test="${pd.locStatus == 'Y' }">selected</c:if> >正常</option>
										<option value="N" <c:if test="${pd.locStatus == 'N' }">selected</c:if> >冻结</option>
										</select>
									</td>
								</tr>
								<tr>
